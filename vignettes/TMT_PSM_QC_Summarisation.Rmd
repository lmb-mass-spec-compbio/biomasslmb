...
Since missing values here are more likely to be genuinely informative (a protein absent from the control pulldown) than in Part A, instead of removing every PSM with any missing value, we retain PSMs with a modest number of missing values and use `MsCoreUtils::robustSummary`, which handles missing values appropriately during aggregation (see the [TMT - comparing protein summarisation approaches](TMT_summarisation_methods.html) vignette for a direct comparison with `sum`).
