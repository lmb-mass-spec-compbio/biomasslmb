...
These two files are the same six acquisitions processed two ways (see [comparing processing pipelines](processing_pipeline_comparison.html)), so for peptides identified by both we can ask directly whether they were assigned to the same protein.
...
That last figure rests on the `r sum(assignments$shared)` shared peptides in this one truncated dataset, so treat it as an indication of the size of the problem rather than as a rate to quote. What it does establish is that the assignment is a decision the search engine makes and not a measurement it reports, since two engines given identical spectra reach different answers often enough to matter.
...
- Master and razor protein assignments are inferences, not measurements. On the `r sum(assignments$shared)` shared peptides in the dataset above, two search engines given identical spectra agreed on the protein less than half the time.
